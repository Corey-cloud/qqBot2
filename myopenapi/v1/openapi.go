// Package v1 是 myopenapi v1 版本的实现。
package v1
...
// Setup 注册
func Setup() {
	myopenapi.Register(myopenapi.APIv1, &openAPI{})
}

// Version 创建当前版本
func (o *openAPI) Version() myopenapi.APIVersion {
	return myopenapi.APIv1
}
...
// Setup 生成一个实例
func (o *openAPI) Setup(token *mytoken.MyToken, inSandbox bool) myopenapi.OpenAPI {
	api := &openAPI{
		token:   token,
...
func (o *openAPI) WithTimeout(duration time.Duration) myopenapi.OpenAPI {
	...
	o.restyClient = resty.New().
		...
		SetPreRequestHook(
			func(client *resty.Client, request *http.Request) error {
				...
				return myopenapi.DoReqFilterChains(request, nil)
			},
		).
		...
		OnAfterResponse(
			func(client *resty.Client, resp *resty.Response) error {
				...
				// 执行请求后过滤器
				if err := myopenapi.DoRespFilterChains(resp.Request.RawRequest, resp.RawResponse); err != nil {
					return err
				}
				traceID := resp.Header().Get(myopenapi.TraceIDKey)
				o.lastTraceID = traceID
				// 非成功含义的状态码，需要返回 error 供调用方识别
				if !myopenapi.IsSuccessStatus(resp.StatusCode()) {
...
func respInfo(resp *resty.Response) string {
	...
	return fmt.Sprintf(
		...
		resp.Request.Method,
		resp.Request.URL,
		resp.Header().Get(myopenapi.TraceIDKey),
		resp.Status(),
		resp.Time(),
		string(bodyJSON),
		string(resp.Body()),
