...
// New 新建一个连接对象
func (c *Client) New(session mydto.Session) websocket.WebSocket {
	return &Client{
...
type messageChan chan *mydto.WSPayload
...
func (c *Client) Listening() error {
	...
	for {
		...
		case <-c.heartBeatTicker.C:
			...
			heartBeatEvent := &mydto.WSPayload{
				WSPayloadBase: mydto.WSPayloadBase{
					OPCode: mydto.WSHeartbeat,
				},
...
func (c *Client) Write(message *mydto.WSPayload) error {
	m, _ := json.Marshal(message)
	log.Infof("%s write %s message, %v", c.session, mydto.OPMeans(message.OPCode), string(m))
...
func (c *Client) Resume() error {
	event := &mydto.WSPayload{
		Data: &mydto.WSResumeData{
			Token:     c.session.Token.GetString(),
			SessionID: c.session.ID,
			Seq:       c.session.LastSeq,
		},
	}
	event.OPCode = mydto.WSResume // 内嵌结构体字段，单独赋值
...
func (c *Client) Identify() error {
	...
	if c.session.Intent == 0 {
		c.session.Intent = mydto.IntentGuilds
	}
	event := &mydto.WSPayload{
		Data: &mydto.WSIdentityData{
			Token:   c.session.Token.GetString(),
...
func (c *Client) Session() *mydto.Session {
	return c.session
}
...
func (c *Client) readMessageToQueue() {
	for {
		...
		log.Infof("%s receive %s message, %s", c.session, mydto.OPMeans(event.OPCode), string(message))
...
func (c *Client) isHandleBuildIn(event *mydto.WSPayload) bool {
	switch event.OPCode {
	case mydto.WSHello: // 接收到 hello 后需要开始发心跳
		c.startHeartBeatTicker(event.RawMessage)
	case mydto.WSHeartbeatAck: // 心跳 ack 不需要业务处理
	case mydto.WSReconnect: // 达到连接时长，需要重新连接，此时可以通过 resume 续传原连接上的事件
		c.closeChan <- errs.ErrNeedReConnect
	case mydto.WSInvalidSession: // 无效的 sessionLog，需要重新鉴权
...
func (c *Client) readyHandler(event *mydto.WSPayload) {
	readyData := &mydto.WSReadyData{}
